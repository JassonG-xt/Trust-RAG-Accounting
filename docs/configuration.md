...
| `EMBEDDING_PROVIDER` | `mock` | Deterministic mock provider by default. |
| `EMBEDDING_DIMENSION` | `64` | Mock embedding dimension. |
...
Qdrant is optional and not required by CI.

## Reranker

| Variable | Default | Notes |
|---|---|---|
| `RERANKER_PROVIDER` | `mock` | Use `none` to disable reranking. |
